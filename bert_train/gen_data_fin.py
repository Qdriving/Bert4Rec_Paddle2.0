...
model_g.add_arg("prop_sliding_window", float, 0.5, "sliding window step size.")
...
def create_instances_threading(all_documents, user, max_seq_length, short_seq_prob,
    ...
    for user in all_documents:
        ...
        instances.extend(create_instances_from_document_train(
            ...
            masked_lm_prob, max_predictions_per_seq, vocab, rng,
            mask_prob))
    write_sample_data(vocab, instances, max_seq_length, max_predictions_per_seq, output_filenamem + "_train_" +str(step) + ".txt")    
    return instances
...
def write_sample_data(vocab, instances, max_seq_length, max_predictions_per_seq, output_file):
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        #fw.writelines(str(masked_lm_weights).replace('[',' ').replace(']', '') + "\n")
...
def main():
    ...
    f_total = open(output_dir + dataset_name+"-train.txt", 'a')
